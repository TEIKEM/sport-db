...
@section('content')
    ...
    <p class="text-sm text-slate-500 mb-4">
        Choisis une équipe, ou deux pour comparer. La page prépare un message avec toutes les stats,
        🟢 vert quand le pourcentage est de 50 % ou plus, 🔴 rouge en dessous. Copie le texte en bas pour le publier toi-même.
    </p>
    ...
    @if(!$team)
        <div class="bg-white rounded-lg shadow p-6 text-slate-500 text-sm">
            Tape le nom d'une équipe pour générer son message. Ajoute une 2e équipe pour un post à deux.
        </div>
    @else
        {{-- Aperçu coloré --}}
        <div class="grid md:grid-cols-2 gap-4 mb-6">
            @foreach(array_filter([$detail1, $detail2]) as $i => $detail)
                ...
                <div class="bg-white rounded-lg shadow overflow-hidden">
                    ...
                    @foreach($detail['categories'] as $category)
                        @php
                            $rows = collect($category['rows'])->filter(fn ($r) => $r['value'] !== '–');
                        @endphp
                        @if($rows->isNotEmpty())
